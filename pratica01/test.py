test = """ MAN(1)                          Manual pager utils                         MAN(1)
...
DESCRIPTION
       ...
       a pre-defined order (see DEFAULTS), and to show only the first page found,
...
EXAMPLES
       ...
       man 'man(7)'
           Display the manual page for macro package man from section  7.   (This
           is another alternative spelling of "man 7 man".  It may be more conve‐
           nient when copying and pasting cross-references to manual pages.  Note
...
OVERVIEW
       ...
       possible to the user.  Changes can be made to the search path, section or‐
       der, output processor, and other behaviours and operations detailed below.
       ...
       to "forget" about the options specified in  $MANOPT,  although  they  must
       still have been valid.

       Manual pages are normally stored in nroff(1) format under a directory such
       as /usr/share/man.  In some installations, there may also be  preformatted
       cat  pages  to  improve  performance.  See manpath(5) for details of where
       ...
       missing or inadequate, please report that to the maintainers of the  pack‐
       age in question.
...
DEFAULTS
       The order of sections to search may be overridden by the environment vari‐
       able  $MANSECT or by the SECTION directive in /etc/manpath.config.  By de‐
       fault it is as follows:

              1 n l 8 3 2 3posix 3pm 3perl 3am 5 4 9 6 7

       The formatted manual page is displayed using a pager.  This can be  speci‐
       fied  in a number of ways, or else will fall back to a default (see option
       -P for details).
       ...
       where string can be any combination of letters described by option -p  be‐
       low.
       ...
       is  executed  instead.   It  gets  passed the manual source file, the pre‐
       processor string, and optionally the device specified with -T or -E as ar‐
       guments.
...
OPTIONS
   ...
   General options
       ...
       --warnings[=warnings]
              ...
              separated list of warning names; if it is not supplied, the default
              is "mac".  See the “Warnings” node in info  groff  for  a  list  of
   ...
   Main modes of operation
       ...
       -K, --global-apropos
              ...
              Search terms may be simple strings (the default),  or  regular  ex‐
       ...
       -w, --where, --path, --location
              ...
              of the source nroff file that would be formatted.  If the -a option
              is also used, then print the locations of  all  source  files  that
       ...
       -W, --where-cat, --location-cat
              ...
              file separated by a space.  If all of -w, -W, and -a are used, then
       ...
       -R encoding, --recode=encoding
              ...
              the  encoding  of  the source file, you can also use manconv(1) di‐
              rectly.  However, this option allows you to convert several  manual
              pages  to  a single encoding without having to explicitly state the
              encoding of each, provided that they were already  installed  in  a
              structure similar to a manual page hierarchy.

              Consider using man-recode(1) instead for converting multiple manual
              pages, since it has an interface designed for bulk  conversion  and
              so can be much faster.
   ...
   Finding manual pages
       -L locale, --locale=locale
              ...
              the search for pages actually begins.  Output such as the help mes‐
       ...
       -m system[,...], --systems=system[,...]
              ...
              they can be accessed using this option.  To  search  for  a  manual
       ...
       -M path, --manpath=path
              ...
              A path specified as a manpath must be the root of a manual page hi‐
              erarchy  structured into sections as described in the man-db manual
              (under "The manual page system").  To  view  manual  pages  outside
       ...
       -e sub-extension, --extension=sub-extension
              ...
              those that accompany the Tcl package, into the main manual page hi‐
              erarchy.  To get around the problem of having two manual pages with
              the  same  name such as exit(3), the Tcl pages were usually all as‐
              signed to section l.  As this is unfortunate, it is now possible to
              put the pages in the correct section, and to assign a specific "ex‐
              tension" to them, in this case, exit(3tcl).   Under  normal  opera‐
              tion, man will display exit(3) in preference to exit(3tcl).  To ne‐
              gotiate this situation and to avoid having to  know  which  section
       ...
       --regex
              ...
              scriptions  matching each page argument as a regular expression, as
              with apropos(1).  Since there is usually no reasonable way to  pick
              a  "best" page when searching for a regular expression, this option
              implies -a.

       --wildcard
              Show all pages with any part of either their  names  or  their  de‐
              scriptions matching each page argument using shell-style wildcards,
              as with apropos(1) --wildcard.  The page argument  must  match  the
              entire  name or description, or match on word boundaries in the de‐
              scription.  Since there is usually no  reasonable  way  to  pick  a
       ...
       --no-subpages
              ...
              mon pattern of programs that implement a number of subcommands, al‐
              lowing them to provide manual pages for each that can  be  accessed
   ...
   Controlling formatted output
       ...
       -r prompt, --prompt=prompt
              ...
              quotes and backslashes in the prompt must be escaped by a preceding
              backslash.   The prompt string may end in an escaped $ which may be
              followed by further options for less.  By default man sets the -ix8
              options.
       ...
       -7, --ascii
              ...
              using the latin1(7) device description with GNU nroff.  This option
              allows pure ascii manual pages to be displayed in  ascii  with  the
              latin1 device.  It will not translate any latin1 text.  The follow‐
              ing table shows the translations performed: some parts  of  it  may
       ...
       --no-hyphenation, --nh
              ...
              so  to lay out words on a line without excessive spacing.  This op‐
              tion disables automatic hyphenation, so words will only be  hyphen‐
       ...
       -H[browser], --html[=browser]
              ...
              is  unset  (usually  lynx).   This option implies -t, and will only
              work with GNU troff.

       -X[dpi], --gxditview[=dpi]
              This option displays the output of groff in a graphical window  us‐
              ing  the  gxditview  program.   The  dpi (dots per inch) may be 75,
       ...
       -Z, --ditroff
              ...
              groff, this option is passed to groff and will suppress the use  of
...
ENVIRONMENT
       ...
       MANSECT
              ...
              what order.  The default is "1 n l 8 3 2 3posix 3pm 3perl 3am 5 4 9
              6  7",  unless  overridden  by  the  SECTION directive in /etc/man‐
              path.config.
       ...
       BROWSER
              ...
              for man --html.  In each command, %s is replaced by a filename con‐
              taining the HTML output from groff, %% is replaced by a single per‐
              cent sign (%), and %c is replaced by a colon (:).
       ...
       MANOPT If $MANOPT is set, it will be parsed prior to  man's  command  line
              and is expected to be in a similar format.  As all of the other man
              specific environment variables can be expressed as command line op‐
              tions,  and are thus candidates for being included in $MANOPT it is
              expected that they will become obsolete.   N.B.   All  spaces  that
              should  be  interpreted as part of an option's argument must be es‐
              caped.
       ...
       MANWIDTH
              ...
              will be formatted with a line length  appropriate  to  the  current
              terminal  (using  the value of $COLUMNS, and ioctl(2) if available,
              or falling back to 80 characters if  neither  is  available).   Cat
              pages  will  only be saved when the default formatting can be used,
              that is when the terminal line length is between 66 and 80  charac‐
              ters.
...
for pal in test.split():
    if pal in ocurrenc:
        ocurrenc[pal] = ocurrenc[pal] + 1
    else:
        ocurrenc[pal] = 1

for chave,valor in ocurrenc.items():
